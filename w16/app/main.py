...
Endpoints:
  ...
  POST /chat/agent    — agentic loop with self-check before responding

Rate limits: 10 req/min on /chat, 5 req/min on /ingest, 3 req/min on /chat/batch, 5 req/min on /chat/agent.
"""
...
# ---------------------------------------------------------------------------
# Agentic chat — self-check before responding
# ---------------------------------------------------------------------------


def _run_agent(body: AgentChatRequest) -> AgentChatResponse:
    if not body.query.strip():
        raise HTTPException(status_code=400, detail="query field is empty.")
    ...
    sources = [
        Source(content=s.get("content", ""), metadata=s.get("metadata", {}))
    ...
    return AgentChatResponse(
        ...
        tokens=AgentTokenUsage(**result["tokens"]),
    )


@app.post("/chat/agent", response_model=AgentChatResponse)
@limiter.limit("5/minute")
async def chat_agent_endpoint(request: Request, body: AgentChatRequest):
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, partial(_run_agent, body))
